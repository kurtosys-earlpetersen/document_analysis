...
Companion reference for `redesigned_factsheet_indesign.xmp`. This document defines every Paragraph Style, Character Style, Table Style, colour swatch, and template setup needed in InDesign so the XML import auto-maps correctly.
...
### Step 2: Import XML
1. **File > Import XML** → select `redesigned_factsheet_indesign.xmp`
